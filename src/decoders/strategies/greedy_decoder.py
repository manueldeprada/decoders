...
class GreedyDecoder(GenerationStrategy):
    """
    ...
    def param_check(cls, config: GenerationConfig):
        if config.num_beams != 1:
            raise ValueError(
                f"{cls.__class__.__name__} cannot be used with num_beams != 1, but num_beams is {config.num_beams}."
            )
        if config.num_beam_groups != 1:
            raise ValueError(
                f"{cls.__class__.__name__} cannot be used with num_beam_groups != 1, "
        ...
        if ConstrainedDecoder.param_test(config):
            raise ValueError(
                f"{cls.__class__.__name__} cannot be used with constraints."
            )
        if ContrastiveDecoder.param_test(config):
            raise ValueError(
                f"{cls.__class__.__name__} cannot be used with contrastive penalties."
            )
